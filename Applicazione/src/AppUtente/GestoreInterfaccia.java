...
public class GestoreInterfaccia implements IGestoreInterfaccia {
	...
	public void setSegnalazione(Segnalazione s) {
		this.segnalazione = s;
	}
	...
	public void posizioneCambiata(String via) {
		...
		if (appManager.setUltimaPosizione(getUtente(), via)) {
			setPosizione(via);
			getGNotifiche().rilevaNotifiche(via); //cerca se vi sono notifiche riguardanti la nuova via e quelle adiacenti
		}
		else {
	...
	public void aggiungiNotificaPush(Notifica n) {
		...
		getGUI().getNotificaVia().setText("In: " + n.getPosizione()); //substring 3 perché i primi 3 caratteri della via contengono il segmento stradale (utilizzato per il calcolo delle vie adiacenti)
	...
	public void setLabelPosizione() {
		if (! getPosizione().isEmpty()) {
			...
			getGUI().getBtnInviaSegn().setEnabled(true);
		}
		else {
			getGUI().getLabelPosizione().setText("Accendi il gps, selezionando una via");
			getGUI().getBtnInviaSegn().setEnabled(false); //se non ha selezionato una via non puo mandare segnalazioni
		}
	}
